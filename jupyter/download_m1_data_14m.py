"""Download multiple years of XAUUSD.sml M1 data in monthly chunks."""
...
SYMBOL = "XAUUSD.sml"
MONTHS = 14  # ~14 months of M1 data
...
def main():
    ...
    out_path = out_dir / "xauusd_m1_14m.csv"
    ...
    start_dt = datetime.fromtimestamp(int(unique[0]["time"]))
    end_dt = datetime.fromtimestamp(int(unique[-1]["time"]))
    days = (int(unique[-1]["time"]) - int(unique[0]["time"])) / 86400

    print(f"\nTotal: {len(unique):,} unique M1 bars")
    print(f"Range: {start_dt.date()} to {end_dt.date()} ({days:.0f} days)")
